...
class DisplayDivision extends Component {
  ...
  addTeamToDivision(event, team) {
    ...
    console.log(
      `term in display division: ${term}, team ${team.division.divCode}`
    );
  }
    ...
    return (
      <Panel>
        <span>
          ...
          <span className="pull-right">{this.props.div.length}</span>
        </span>

        <div>{divisionList}</div>
      </Panel>
    );
  }
}
